...
#define LOAD_DBC(store, file, dbtable) LoadDBC(availableDbcLocales, bad_dbc_files, store, dbcPath, file, dbtable)
    ...
    LOAD_DBC(sAreaTableStore,                       "AreaTable.dbc",                        NULL);
    LOAD_DBC(sAchievementStore,                     "Achievement.dbc",                      "achievement_dbc");
    LOAD_DBC(sAchievementCategoryStore,             "Achievement_Category.dbc",             NULL);
    LOAD_DBC(sAchievementCriteriaStore,             "Achievement_Criteria.dbc",             NULL);
    LOAD_DBC(sAreaGroupStore,                       "AreaGroup.dbc",                        NULL);
    LOAD_DBC(sAreaPOIStore,                         "AreaPOI.dbc",                          NULL);
    LOAD_DBC(sAuctionHouseStore,                    "AuctionHouse.dbc",                     NULL);
    LOAD_DBC(sBankBagSlotPricesStore,               "BankBagSlotPrices.dbc",                NULL);
    LOAD_DBC(sBattlemasterListStore,                "BattlemasterList.dbc",                 NULL);
    LOAD_DBC(sBarberShopStyleStore,                 "BarberShopStyle.dbc",                  NULL);
    LOAD_DBC(sCharStartOutfitStore,                 "CharStartOutfit.dbc",                  NULL);
    LOAD_DBC(sCharSectionsStore,                    "CharSections.dbc",                     NULL);
    LOAD_DBC(sCharTitlesStore,                      "CharTitles.dbc",                       NULL);
    LOAD_DBC(sChatChannelsStore,                    "ChatChannels.dbc",                     NULL);
    LOAD_DBC(sChrClassesStore,                      "ChrClasses.dbc",                       NULL);
    LOAD_DBC(sChrRacesStore,                        "ChrRaces.dbc",                         NULL);
    LOAD_DBC(sCinematicCameraStore,                 "CinematicCamera.dbc",                  NULL);
    LOAD_DBC(sCinematicSequencesStore,              "CinematicSequences.dbc",               NULL);
    LOAD_DBC(sCreatureDisplayInfoStore,             "CreatureDisplayInfo.dbc",              NULL);
    LOAD_DBC(sCreatureDisplayInfoExtraStore,        "CreatureDisplayInfoExtra.dbc",         NULL);
    LOAD_DBC(sCreatureFamilyStore,                  "CreatureFamily.dbc",                   NULL);
    LOAD_DBC(sCreatureModelDataStore,               "CreatureModelData.dbc",                NULL);
    LOAD_DBC(sCreatureSpellDataStore,               "CreatureSpellData.dbc",                NULL);
    LOAD_DBC(sCreatureTypeStore,                    "CreatureType.dbc",                     NULL);
    LOAD_DBC(sCurrencyTypesStore,                   "CurrencyTypes.dbc",                    NULL);
    LOAD_DBC(sDestructibleModelDataStore,           "DestructibleModelData.dbc",            NULL);
    LOAD_DBC(sDungeonEncounterStore,                "DungeonEncounter.dbc",                 NULL);
    LOAD_DBC(sDurabilityCostsStore,                 "DurabilityCosts.dbc",                  NULL);
    LOAD_DBC(sDurabilityQualityStore,               "DurabilityQuality.dbc",                NULL);
    LOAD_DBC(sEmotesStore,                          "Emotes.dbc",                           NULL);
    LOAD_DBC(sEmotesTextStore,                      "EmotesText.dbc",                       NULL);
    LOAD_DBC(sEmotesTextSoundStore,                 "EmotesTextSound.dbc",                  NULL);
    LOAD_DBC(sFactionStore,                         "Faction.dbc",                          NULL);
    LOAD_DBC(sFactionTemplateStore,                 "FactionTemplate.dbc",                  "factiontemplate_dbc");
    LOAD_DBC(sGameObjectArtKitStore,                "GameObjectArtKit.dbc",                 NULL);
    LOAD_DBC(sGameObjectDisplayInfoStore,           "GameObjectDisplayInfo.dbc",            NULL);
    LOAD_DBC(sGemPropertiesStore,                   "GemProperties.dbc",                    NULL);
    LOAD_DBC(sGlyphPropertiesStore,                 "GlyphProperties.dbc",                  NULL);
    LOAD_DBC(sGlyphSlotStore,                       "GlyphSlot.dbc",                        NULL);
    ...
    LOAD_DBC(sHolidaysStore,                        "Holidays.dbc",                         NULL);
    LOAD_DBC(sItemStore,                            "Item.dbc",                             NULL);
    LOAD_DBC(sItemBagFamilyStore,                   "ItemBagFamily.dbc",                    NULL);
    LOAD_DBC(sItemDisplayInfoStore,                 "ItemDisplayInfo.dbc",                  NULL);
    //LOAD_DBC(sItemCondExtCostsStore,              "ItemCondExtCosts.dbc",                 "itemcondextcosts_dbc");
    LOAD_DBC(sItemExtendedCostStore,                "ItemExtendedCost.dbc",                 NULL);
    LOAD_DBC(sItemLimitCategoryStore,               "ItemLimitCategory.dbc",                NULL);
    LOAD_DBC(sItemRandomPropertiesStore,            "ItemRandomProperties.dbc",             NULL);
    LOAD_DBC(sItemRandomSuffixStore,                "ItemRandomSuffix.dbc",                 NULL);
    LOAD_DBC(sItemSetStore,                         "ItemSet.dbc",                          NULL);
    LOAD_DBC(sLFGDungeonStore,                      "LFGDungeons.dbc",                      NULL);
    LOAD_DBC(sLightStore,                           "Light.dbc",                            NULL);
    LOAD_DBC(sLiquidTypeStore,                      "LiquidType.dbc",                       NULL);
    LOAD_DBC(sLockStore,                            "Lock.dbc",                             NULL);
    LOAD_DBC(sMailTemplateStore,                    "MailTemplate.dbc",                     NULL);
    LOAD_DBC(sMapStore,                             "Map.dbc",                              NULL);
    LOAD_DBC(sMapDifficultyStore,                   "MapDifficulty.dbc",                    NULL);
    LOAD_DBC(sMovieStore,                           "Movie.dbc",                            NULL);
    LOAD_DBC(sNamesReservedStore,                   "NamesReserved.dbc",                    NULL);
    LOAD_DBC(sNamesProfanityStore,                  "NamesProfanity.dbc",                   NULL);
    LOAD_DBC(sOverrideSpellDataStore,               "OverrideSpellData.dbc",                NULL);
    LOAD_DBC(sPowerDisplayStore,                    "PowerDisplay.dbc",                     NULL);
    LOAD_DBC(sPvPDifficultyStore,                   "PvpDifficulty.dbc",                    NULL);
    LOAD_DBC(sQuestXPStore,                         "QuestXP.dbc",                          NULL);
    LOAD_DBC(sQuestFactionRewardStore,              "QuestFactionReward.dbc",               NULL);
    LOAD_DBC(sQuestSortStore,                       "QuestSort.dbc",                        NULL);
    LOAD_DBC(sRandomPropertiesPointsStore,          "RandPropPoints.dbc",                   NULL);
    LOAD_DBC(sScalingStatDistributionStore,         "ScalingStatDistribution.dbc",          NULL);
    LOAD_DBC(sScalingStatValuesStore,               "ScalingStatValues.dbc",                NULL);
    LOAD_DBC(sSkillLineStore,                       "SkillLine.dbc",                        NULL);
    LOAD_DBC(sSkillLineAbilityStore,                "SkillLineAbility.dbc",                 NULL);
    LOAD_DBC(sSkillRaceClassInfoStore,              "SkillRaceClassInfo.dbc",               NULL);
    LOAD_DBC(sSkillTiersStore,                      "SkillTiers.dbc",                       NULL);
    LOAD_DBC(sSoundEntriesStore,                    "SoundEntries.dbc",                     NULL);
    LOAD_DBC(sSpellStore,                           "Spell.dbc",                            "spell_dbc");
    LOAD_DBC(sSpellCastTimesStore,                  "SpellCastTimes.dbc",                   NULL);
    LOAD_DBC(sSpellCategoryStore,                   "SpellCategory.dbc",                    NULL);
    LOAD_DBC(sSpellDifficultyStore,                 "SpellDifficulty.dbc",                  "spelldifficulty_dbc");
    LOAD_DBC(sSpellDurationStore,                   "SpellDuration.dbc",                    NULL);
    LOAD_DBC(sSpellFocusObjectStore,                "SpellFocusObject.dbc",                 NULL);
    LOAD_DBC(sSpellItemEnchantmentStore,            "SpellItemEnchantment.dbc",             NULL);
    LOAD_DBC(sSpellItemEnchantmentConditionStore,   "SpellItemEnchantmentCondition.dbc",    NULL);
    LOAD_DBC(sSpellRadiusStore,                     "SpellRadius.dbc",                      NULL);
    LOAD_DBC(sSpellRangeStore,                      "SpellRange.dbc",                       NULL);
    LOAD_DBC(sSpellRuneCostStore,                   "SpellRuneCost.dbc",                    NULL);
    LOAD_DBC(sSpellShapeshiftFormStore,             "SpellShapeshiftForm.dbc",              NULL);
    LOAD_DBC(sSpellVisualStore,                     "SpellVisual.dbc",                      NULL);
    LOAD_DBC(sStableSlotPricesStore,                "StableSlotPrices.dbc",                 NULL);
    LOAD_DBC(sSummonPropertiesStore,                "SummonProperties.dbc",                 NULL);
    LOAD_DBC(sTalentStore,                          "Talent.dbc",                           NULL);
    LOAD_DBC(sTalentTabStore,                       "TalentTab.dbc",                        NULL);
    LOAD_DBC(sTaxiNodesStore,                       "TaxiNodes.dbc",                        NULL);
    LOAD_DBC(sTaxiPathStore,                        "TaxiPath.dbc",                         NULL);
    LOAD_DBC(sTaxiPathNodeStore,                    "TaxiPathNode.dbc",                     NULL);
    LOAD_DBC(sTeamContributionPointsStore,          "TeamContributionPoints.dbc",           NULL);
    LOAD_DBC(sTotemCategoryStore,                   "TotemCategory.dbc",                    NULL);
    LOAD_DBC(sTransportAnimationStore,              "TransportAnimation.dbc",               NULL);
    LOAD_DBC(sTransportRotationStore,               "TransportRotation.dbc",                NULL);
    LOAD_DBC(sVehicleStore,                         "Vehicle.dbc",                          NULL);
    LOAD_DBC(sVehicleSeatStore,                     "VehicleSeat.dbc",                      NULL);
    LOAD_DBC(sWMOAreaTableStore,                    "WMOAreaTable.dbc",                     NULL);
    LOAD_DBC(sWorldMapAreaStore,                    "WorldMapArea.dbc",                     NULL);
    LOAD_DBC(sWorldMapOverlayStore,                 "WorldMapOverlay.dbc",                  NULL);
